...
pub fn stash_pop(repo: &mut Repository) -> Result<()> {
    ...
    // Parse dirty tree hash out of the message
    let dirty_tree_hash = message
        .split("dirty=")
        .nth(1)
        .and_then(|s| crate::hash::hex_to_hash(s.trim()).ok());

    //
    //
    // Restore staged state into index
    //
    //

    let staged_object  = repo.read_object(&tree_hash)?;
    let staged_tree_id = staged_object.try_as_tree_id()?;
    ...
    if let Some(dirty_hash) = dirty_tree_hash {
        ...
        for j in 0..m {
            ...
            repo.storage.evict_pages(raw);
            //
            // Don't update index for dirty files they should show as modified.
            //
        }
    }
    ...
    //
    //
    // Remove stash ref and shift down.
    //
    //
...
fn shift_stash_refs_down(repo: &Repository) -> Result<()> {
    ...
    // Rename lowest first to avoid clobbering.
